...
<div class="container py-5" id="mainContainer">
    <div class="row mb-4">
        <div class="col-md-8">
            <h4 class="h4 fw-bold">{{ $challenge->title }}({{ $challenge->marks }} Marks)</h4>
            <p class="text-muted">Difficulty: <span class="badge bg-primary">{{ ucfirst($challenge->difficulty) }}</span></p>
        </div>
        <div class="col-md-2 text-center">
            <div id="timer" class="h1 text-danger">{{ gmdate("i:s", $challenge->time * 60) }}</div>
...
<script>
    let time = {{ $challenge->time * 60 }}; // Time from the database in seconds
    let timerInterval;
